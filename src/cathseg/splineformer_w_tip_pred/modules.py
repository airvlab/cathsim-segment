...
class TipPredictor(nn.Module):
    def __init__(self, num_channels, image_size):
        ...
        # Define a series of convolutional layers followed by fully connected layers
        # to process images of arbitrary size
        self.conv_layers = nn.Sequential(
            nn.Conv2d(num_channels, 64, kernel_size=3, stride=1, padding=1),  # Conv Layer 1
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # Reduce the spatial size by half
            nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1),  # Conv Layer 2
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # Reduce the spatial size by half again
            nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1),  # Conv Layer 3
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # Reduce the spatial size by half again
            nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),  # Conv Layer 4
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # Reduce the spatial size by half again
        )
        ...
        # Fully connected layers to predict the x, y coordinates
        self.fc_layers = nn.Sequential(
            nn.Linear(flattened_size, 1024),  # Hidden fully connected layer
            nn.ReLU(),
            nn.Linear(1024, 512),  # Another fully connected layer
            nn.ReLU(),
            nn.Linear(512, 2),  # Output layer predicting 2D coordinates (x, y)
        )
