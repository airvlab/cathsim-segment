...
class FSIM_base(nn.Module):
    def __init__(self):
        ...
        self.epsilon = 0.0001  # Used to prevent division by zero
        self.pi = math.pi
    ...
    def phasecong2(self, img):
        ...
        imagefft_repeat = pt.cat(self.nscale * self.norient * [imagefft], dim=1).view(
            -1, self.nscale, self.norient, rows, cols, 2
        )
...
class FSIMc(FSIM_base, nn.Module):
    ...
    def forward(self, imgr, imgd):
        ...
        gradientSimMatrix = self.calculate_gradient_sim(gradientMap1, gradientMap2)
        gradientSimMatrix = gradientSimMatrix.view(PCSimMatrix.size())
        FSIMc = self.calculate_fsimc(
            I1.squeeze(), Q1.squeeze(), I2.squeeze(), Q2.squeeze(), gradientSimMatrix, PCSimMatrix, PCm
        )
