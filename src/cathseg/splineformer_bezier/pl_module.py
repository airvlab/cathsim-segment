...
from cathseg.splineformer_bezier.model import SplineTransformer as Model
...
class SplineFormer(pl.LightningModule):
    def __init__(
        ...
        self.tip_predictor = modules.TipPredictorMobileNetV2(num_dim=2, image_size=img_size, num_channels=num_channels)
    ...
    def _compute_loss(
        ...
        loss = self.lambda_coeff * loss_coeff + self.lambda_eos * loss_eos

        return dict(coeff=loss_coeff, eos=loss_eos, total_loss=loss)
