...
class SplineTransformer(nn.Module):
    def __init__(
        ...
        # self.fc_seq = nn.Linear(d_model, 1 + dim_pts)  # Predicting n_dim control and 1D knots
        self.knot_pred = nn.Linear(d_model, dim_pts)
        self.coeff_pred = nn.Linear(d_model, 1)
    ...
    def forward(
        ...
        coeff = self.coeff_pred(output)
        knots = self.knot_pred(output)
        seq = torch.cat([knots, coeff], dim=-1)
