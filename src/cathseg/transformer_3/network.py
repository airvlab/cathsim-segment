...
class ImageToSequenceTransformer(pl.LightningModule):
    def __init__(
        ...
        self.fc_seq = nn.Sequential(nn.Linear(d_model, 3))  # Predicting 2D control and 1D knots
        ...
        if batch_idx == 0:
            self.training_step_output = [
                dict(
                    ...
                    t_pred=target_seq[i, :, 0:1],
                    c_true=pred_seq[i, :, 1:3],
    ...
    def _log(self, loss_pts, loss_eos, loss, tag):
        self.log(f"{tag}/loss_pts", loss_pts.sum())
        self.log(f"{tag}/loss_eos", loss_eos.sum())
        self.log(f"{tag}/loss", loss)

    def training_step(self, batch, batch_idx):
        loss_pts, loss_eos, loss = self._step(batch, batch_idx)
        self._log(loss_pts, loss_eos, loss, "train")
        return loss

    def validation_step(self, batch, batch_idx):
        loss_pts, loss_eos, loss = self._step(batch, batch_idx)
        self._log(loss_pts, loss_eos, loss, "val")
        return loss
